...
export default class BufferList extends Duplex {
    ...
    constructor(buf?: Buffer | BufferList | (Buffer | BufferList | string)[] | Function | number | string | Uint8Array) {
        super();
        this._bufs = [];
        this.length = 0;
    ...
    // _new(buf?: Buffer | Buffer[] | BufferList | BufferList[]): BufferList {
    //     return new BufferList(buf);
    // };

    _offset(offset: number): [number, number] | void {
        if (offset === 0) {
            return [0, 0];
        }
        let tot = 0;
    ...
    copy (dst?: Buffer | null, dstStart?: number, srcStart?: number, srcEnd?: number): Buffer {
        ...
        if (dst === null || dst === undefined) {
            dst = Buffer.alloc(srcEnd - srcStart)
        }
        const copy = !!dst
    ...
    consume (bytes: number): BufferList{
    ...
    append (buf: Buffer | BufferList | (Buffer | BufferList | string)[] | number | string | Uint8Array): BufferList {
        if (buf == null) {
            return this
        }
